...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
                    ...
                    public void onClick(View v) {
                        if (config.isMaxSelectEnabledMask) {
                            if (config.isWithVideoImage) {
                                int selectedCount = PictureImageGridAdapter.this.getSelectedSize();
                                int videoSize = 0;
                                ...
                                if (isNotOption) {
                                    PictureImageGridAdapter.this.showPromptDialog(errorMsg);
                                    return;
                                }
                            } else {
                                if (!contentHolder.tvCheck.isSelected() && PictureImageGridAdapter.this.getSelectedSize() >= config.maxSelectNum) {
                                    String msg = StringUtils.getMsg(context, image.getMimeType(), config.maxSelectNum);
                                    PictureImageGridAdapter.this.showPromptDialog(msg);
                                    return;
                                }
                            }
                        }
                    ...
                    if (eqResult) {
                        if (PictureMimeType.isHasVideo(image.getMimeType())) {
                            if (config.videoMinSecond > 0 && image.getDuration() < config.videoMinSecond) {
                                ...
                                PictureImageGridAdapter.this.showPromptDialog(context.getString(R.string.picture_choose_min_seconds, config.videoMinSecond / 1000));
                                return;
                            }
                            if (config.videoMaxSecond > 0 && image.getDuration() > config.videoMaxSecond) {
                                // The length of the video exceeds the specified length
                                PictureImageGridAdapter.this.showPromptDialog(context.getString(R.string.picture_choose_max_seconds, config.videoMaxSecond / 1000));
                                return;
                            }
                        }
    ...
    private void showPromptDialog(String content) {
        if (PictureSelectionConfig.onChooseLimitCallback != null) {
            ...
            final PictureCustomDialog dialog = new PictureCustomDialog(context, R.layout.picture_prompt_dialog);
